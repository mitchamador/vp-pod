...
Esp32A2dpBluetoothSource::Esp32A2dpBluetoothSource(BluetoothA2DPSink &a2dpSink, AudioStream *rawOutputStream)
    : _a2dp(a2dpSink), _rawOutputStream(rawOutputStream)
{
    _instance = this;
}
...
void Esp32A2dpBluetoothSource::_connectionStateChangedTrampoline(esp_a2d_connection_state_t state, void *ptr)
{
    ...
    case ESP_A2D_CONNECTION_STATE_CONNECTING:
        ...
        {
            self->_sink->onConnectionStateChanged(BtConnectionState::Connecting, nullptr);
        }
        break;
    case ESP_A2D_CONNECTION_STATE_CONNECTED:
        ...
        {
#ifdef USE_PEER_NAME
            self->_sink->onConnectionStateChanged(BtConnectionState::Connected, self->_a2dp.get_peer_name());
#else
            self->_sink->onConnectionStateChanged(BtConnectionState::Connected, nullptr);
#endif
        }
        break;
    case ESP_A2D_CONNECTION_STATE_DISCONNECTED:
        ...
        if (self->_sink != nullptr)
        {
            self->_sink->onConnectionStateChanged(BtConnectionState::Disconnected, nullptr);
        }
...
{
    ESP_LOGD("BT_SRC", connected ? "AVRC Connection state: connected" : "AVRC Connection state: disconnected");
}
...
{
    ESP_LOGI("BT_SRC", "get volume from NVS");
...
void Esp32A2dpBluetoothSource::_nvsSetVolume(uint8_t volume)
{
    ...
    {
        ESP_LOGI("BT_SRC", "set volume to NVS");
...
#ifndef AUDIOKIT
    if (self->_rawOutputStream != nullptr)
    {
        self->_rawOutputStream->write(data, length);
    }
#endif

    ...
    else
    {
        ...
        case L0x04_SetPlayStatusChangeNotification: // Turns on basic notifications
        {
            ...
            if (esp->playStatusNotificationState == NOTIF_ON) {
                if (xTimerIsTimerActive(esp->_playPositionTimer) != pdTRUE) {
                    xTimerStart(esp->_playPositionTimer, 0);
                }
            } else if (esp->playStatusNotificationState == NOTIF_OFF) {
                if (xTimerIsTimerActive(esp->_playPositionTimer) == pdTRUE) {
                    xTimerStop(esp->_playPositionTimer, 0);
                }
            }
...
#if TOTAL_NUM_TRACKS == 3
            ESP_LOGD(IPOD_TAG, "Current track index %d New index %d Pending Meta %d Timestamp: %d --> PREV ", _currentTrackIndex, esp->currentTrackIndex, (esp->trackChangeAckPending > 0x00), esp->trackChangeTimestamp);
#else
        ...
        case L0x04_PlayControl: // Basic play control. Used for Prev, pause and play
        {
            ...
            case PB_CMD_NEXT: // Next track
            {
                ...
                esp->trackChangeTimestamp = platform::time_now_ms();
#if TOTAL_NUM_TRACKS == 3
                ESP_LOGD(IPOD_TAG, "New index %d Pending Meta %d Timestamp: %d --> EXPLICIT NEXT TRACK", esp->currentTrackIndex, (esp->trackChangeAckPending > 0x00), esp->trackChangeTimestamp);
#else
            ...
            {
#if TOTAL_NUM_TRACKS == 3
                ESP_LOGD(IPOD_TAG, "Current index %d --> EXPLICIT SINGLE PREV TRACK", esp->currentTrackIndex);
#else
        ...
        case L0x04_SetShuffle: // Set Shuffle state
        {
...
#endif
            {
                ...
                // SHUFFLE_TOGGLE_WINDOW_MS calls shuffleSwitch().
                if (_shuffleRequest != esp->shuffleStatus)
                {
                    ...
                    if (esp->_lastShuffleToggleTimestamp != INVALID_TIMESTAMP &&
                        (now - esp->_lastShuffleToggleTimestamp) < SHUFFLE_TOGGLE_WINDOW_MS)
                    {
                        esp->shuffleSwitch();
